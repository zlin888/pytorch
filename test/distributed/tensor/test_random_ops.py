...
class DistTensorRandomOpCompileTest(DTensorTestBase):
    def _run_eager_and_compiled(self, fn, create_input, num_runs):
        """Run fn both eagerly and compiled, returning results and RNG states.

        Resets RNG to the same seed before each mode so results are comparable.
        Also verifies the compiled graph contains run_dtensor_rng_op and that
        RNG state advances after each call.
        """
        from torch._dynamo.testing import AotEagerAndRecordGraphs

        def run(compiled_fn=None):
            # reset RNG to the same seed before each mode
            torch.manual_seed(0)
            results = []
            rng_states = [torch.cuda.get_rng_state()]
            for _ in range(num_runs):
                x = create_input()
                result = compiled_fn(x) if compiled_fn else fn(x)
        ...
        eager_results, eager_rng_states = run()
        backend = AotEagerAndRecordGraphs()
        compiled_fn = torch.compile(fn, backend=backend, fullgraph=True)
        compiled_results, compiled_rng_states = run(compiled_fn)
