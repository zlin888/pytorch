...
class OffsetBasedRNGTracker(_RNGStateTracker):
    ...
    def __init__(
        ...
        if run_state_sync:
            ...
            logger.warning(
                "OffsetBasedRNGTracker is skipping RNG state synchronization "
                "(e.g. during tracing). Please ensure all ranks have the same "
                "RNG state by calling `torch.manual_seed()` with the same seed "
                "on every rank before running DTensor random operations."
            )
